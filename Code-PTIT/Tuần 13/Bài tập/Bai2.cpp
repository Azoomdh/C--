...
int main(){
    ...
    fstream ioBanRo("banro.txt");
    
    if(ioBanRo.is_open()==true ){
        string motDong;
        while(getline(ioBanRo, motDong)) {
            input = input + motDong;
        }

        MaHoaMotChieu maHoaMotChieu(input);
        string hexCode = maHoaMotChieu.hashStringToHex();

        fstream ioBanMa("banma.bam");

        if(ioBanMa.is_open()==true ){
            ioBanMa << hexCode << endl;
        }
    }
